...
use rand::{Rng, SeedableRng, distributions::{Distribution, WeightedIndex}};
...
pub struct WeightedDistribution<T> {
    items: Vec<(T, f32)>,
    alias_index: Option<WeightedIndex<f32>>,
}
...
impl<T: Clone + std::hash::Hash + Eq> WeightedDistribution<T> {
    pub fn new(items: Vec<(T, f32)>) -> Self {
        ...
        let alias_index = WeightedIndex::new(weights).ok();
...
impl ClusterProcessor {
    ...
    pub fn process_cluster(&self, cluster: &mut DataCluster) -> Vec<u8> {
